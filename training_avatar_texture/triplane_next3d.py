...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, c, v, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        ray_origins, ray_directions = self.ray_sampler(cam2world_matrix, intrinsics, self.neural_rendering_resolution*2)
        N, M, _ = ray_origins.shape
        feature_samples, depth_samples, weights_samples, render_out, render_points = self.renderer(blended_planes, self.decoder, ray_origins, ray_directions, self.rendering_kwargs) # channels last
        # Reshape into 'raw' neural-rendered image
        H = W = self.neural_rendering_resolution*2
        depth_image = depth_samples.permute(0, 2, 1).reshape(N, 1, H, W)
        
        # Perform volume rendering again for RGB image
        ray_origins, ray_directions = self.ray_sampler(cam2world_matrix, intrinsics, neural_rendering_resolution)
        N, M, _ = ray_origins.shape
        feature_samples, depth_samples, weights_samples, render_out, render_points = self.renderer(blended_planes, self.decoder, ray_origins, ray_directions, self.rendering_kwargs) # channels last
        # Reshape into 'raw' neural-rendered image
        ...
        return {'image': sr_image, 'image_raw': rgb_image, 'image_depth': depth_image, 'nerf_out': render_out, 'nerf_pts': render_points}
